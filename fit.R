...
##RK code#####
vdat1 <- vdat%>%mutate(YC=Year-Age,log.index=ifelse(wt==0,NA,log(index)))
all_equal(vdat,vdat1)
###
...
####RK code for P1
ggplot(data=vdat1,aes(x=Year,y=index,group=Age,color=factor(Age)))+
  geom_line()+
  # scale_y_continuous(limits=c(0,6))+#this removes the data then plot
...
##RK code for P3
vdat2 <- mutate(vdat1,zero=ifelse(index==0,"Y","N"))
ggplot(data=vdat2,aes(x=Year,y=factor(Age)))+
  geom_point(aes(size=index,shape=zero,color=zero))+
